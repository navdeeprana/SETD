...
# Define parameters
nx, lx = 1024, 320π
tmax, h, saveat, save_after = 1.0e+3, 1.0e-1, 1.0e+1, 0.0e+0
seed = 42
pars = (b = 0.0, c = 0.5, D = 1.0e-2, q = 16 * (2π / lx));
...
# Analysis
nx, lx = 1024, 320π
tmax, h, saveat, save_after = 2.0e+5, 2.0e-2, 2.0e+2, 0.0
...
axes[1].limits = (-1, 1, 5.0e-3, 1.0e+3)
axes[1].xscale = Makie.pseudolog10

ax = axes[2]
ax.limits = (1.5π / grid.lx, 1.8, 1.0e-2, 5.0e+2)
ax.xscale = log10

labels = [L"D=0.01", L"D=0.05", L"D=0.10"]
for (D, m, c, l) in zip([0.01, 0.05, 0.1], [:circle, :rect, :diamond], colors, labels)
...
k = 0.02:0.1:1.0
lines!(k, (@. 1.0e-1 / k^2), color = :black)
text!(ax, L"$k^{-2}$"; position = (1.5e-1, 1.0e+1), fontsize = 30)
