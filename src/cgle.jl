...
function nonlinear!(sim)
    ...
    return @. duk = duk - 1im * pars.b * grid.k2 * uk
end
...
function setd1!(sim, int)
    ...
    (; uk, duk, nl) = vars
    (; f1, f2, f3) = int
    return @inbounds for i in 1:length(grid.k)
        if grid.kin[i]
...
function myjldsave(fout, group; kwargs...)
    return jldsave(fout; (Symbol("$(group)/$(k)") => v for (k, v) in pairs(kwargs))...)
end
